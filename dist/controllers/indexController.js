...
const seeHomepageCb = async function (t) {
    const seeHomepageQuery = async function () {
        ...
        const getFeaturedUrls = async function () {
            ...
            const featuredCompetitionsUrls = await getFeaturedCompetitionUrls(t, featuredCompetitionNames).catch((err) => {
                throw err;
            });
            const featuredPlayersUrls = await getFeaturedPlayerUrls(t, featuredPlayerNames).catch((err) => {
                throw err;
            });
            const featuredTeamsUrls = await getFeaturedTeamUrls(t, featuredTeamNames).catch((err) => {
                throw err;
            });
        ...
        const getDissociatedUrls = async function () {
            const freeCompetitionModels = await getAllDissociatedCompetitions(t).catch((err) => {
                throw err;
            });
            const freePlayerModels = await getAllDissociatedPlayers(t).catch((err) => {
                throw err;
            });
    ...
    const populateSeeHomepageResults = function () {
        if (results.featuredCompetitions &&
