...
let competitionDataResults: resultsGenerator.competitionDataResults | null =
  null;
let preFormCreateCompetitionResults: resultsGenerator.preFormCreateCompetitionResults | null =
  null;
let postFormCreateCompetitionResults:
  | resultsGenerator.postFormCreateCompetitionResults
  | null
  | undefined = null;
let preFormUpdateCompetitionResults: resultsGenerator.preFormUpdateCompetitionResults | null =
  null;
let postFormUpdateCompetitionResults: resultsGenerator.postFormUpdateCompetitionResults | null =
  null;
let seeCompetitionResults: resultsGenerator.seeCompetitionResults | null = null;
let seeCompetitionIndexResults: resultsGenerator.seeCompetitionIndexResults | null =
  null;
...
export const competitionIndexSignal = async function (
  ...
  const newIndexData = async function () {
    const generateData = async function () {
      await transactionWrapper(competitionIndexDataCb, next).catch(
        (err: Error) => {
          throw err;
        }
      );
...
const competitionIndexDataCb = async function (t: Transaction) {
  const competitionIndexDataQuery = async function () {
    ...
    const allCompetitions = await getAllCompetitions(t).catch((err: Error) => {
      throw err;
    });
    const associatedCompetitionsPromises =
      allCompetitions && allCompetitions.length > 0
        ? allCompetitions.map(
            (competition) => async () =>
              await (competition as any).countTeams({ transaction: t })
          )
        : [];
    const teamsCount =
    ...
    const teamsPromises = associatedCompetitions.map(
      (competition) => async () =>
        await (competition as any).getTeams({ transaction: t })
    );
    const teamsSets =
    ...
    if (
      ...
      const sortDetails = function (seasonName: string) {
        ...
        competitionData[seasonName].sort(function (x, y) {
          return (namesCopy as string[]).indexOf(x["name"]) <
            (namesCopy as string[]).indexOf(y["name"])
            ? -1
            : 1;
        });
        namesCopy = null;
  ...
  const results = await competitionIndexDataQuery().catch((err: Error) => {
    throw err;
  });
  if (results) {
...
export const competitionIndexData = async function (
  _req: Request,
  res: Response,
  next: NextFunction
) {
  await transactionWrapper(competitionIndexDataCb, next).catch(function (
    err: Error
  ) {
    throw err;
  });
...
export const deleteCompetition = [
  async function (
    ...
    await transactionWrapper(deleteCompetitionCb, next).catch(function (
      error: Error
    ) {
      next(error);
    });
...
const seeCompetitionCb = async function (t: Transaction): Promise<void> {
  const sortCompetitionData = function (
    teams: string[],
    teamUrls: string[],
  ...
  const seeCompetitionQuery = async function () {
    ...
    const teams = await (competition as any)
      ?.getTeams({
...
export const seeCompetition = [
  async function (
    ...
    await transactionWrapper(seeCompetitionCb, next).catch(function (
      error: Error
    ) {
      next(error);
    });
  ...
  async function (
    ...
    const updateDatabaseWithNewRankings = async function () {
      if (seeCompetitionResults) {
        await transactionWrapper(getCompetitionQuery, next).catch(
          (err: Error) => {
            throw err;
          }
        );
        if (competitionResult) {
          ...
          const applyLatestRankings = async function (t: Transaction) {
            if (postFormUpdateCompetitionResults) {
              await applyRanking(
                competitionResult as CompetitionModel,
                postFormUpdateCompetitionResults,
                t
              );
            }
          };
          await transactionWrapper(applyLatestRankings, next).catch(
            (err: Error) => {
              throw err;
            }
          );
    ...
    await updateDatabaseWithNewRankings().catch((err: Error) => {
      throw err;
    });
    next();
  },
...
const seeCompetitionIndexCb = async function (t: Transaction) {
  const competitionIndexDataProcessor = function (
  ...
  const newCompetitionIndexData = async function () {
    const generateData = async function () {
      await competitionIndexDataCb(t).catch((err: Error) => {
        throw err;
      });
      writeIndexData(competitionDataResults);
    };
    ...
    await generateData();
    const data = competitionIndexDataProcessor(
      competitionDataResults as resultsGenerator.competitionDataResults
    );
    nullifyData();
...
export const seeCompetitionIndex = async function (
  _req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
  await transactionWrapper(seeCompetitionIndexCb, next).catch(function (
    error: Error
  ) {
    next(error);
  });
...
const preFormCreateCompetitionCb = async function (
  t: Transaction
): Promise<void> {
  ...
  const populatePreFormCreateCompetition = function () {
    if (results) {
      ...
      Object.assign(
        preFormCreateCompetitionResults as resultsGenerator.preFormCreateCompetitionResults,
        { teams: teamNames },
        { seasons: getSeasons() }
      );
    } else {
...
export const preFormCreateCompetition = async function (
  _req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
...
const postFormCreateCompetitionCb = async function (t: Transaction) {
  ...
  const getRelevantTeams = async function () {
    ...
    const teamNames = (
      postFormCreateCompetitionResults as resultsGenerator.postFormCreateCompetitionResults
    ).chosenTeams;
    const chosenSeason = (
      postFormCreateCompetitionResults as resultsGenerator.postFormCreateCompetitionResults
    ).season;
  ...
  const latestCompetition = await createCompetition().catch(function (
    err: Error
  ) {
    throw err;
  });
...
export const postFormCreateCompetition = [
  ...
  async (_req: Request, res: Response, next: NextFunction) => {
    const goToCompetitionPage = async function () {
      try {
        const latestCode = await Competition.max("code").catch(function (
          error: Error
        ) {
          throw error;
        });
...
const preFormUpdateCompetitionCb = async function (
  t: Transaction
): Promise<void> {
  ...
  const populatePreFormUpdateCompetition = function () {
    if (results.competition) {
      Object.assign(
        preFormUpdateCompetitionResults as resultsGenerator.preFormUpdateCompetitionResults,
        results.competition.get(),
...
export const preFormUpdateCompetition = async function (
  req: Request,
  res: Response,
  next: NextFunction
): Promise<void> {
...
const postFormUpdateCompetitionCb = async function (
  t: Transaction
): Promise<void> {
  const { applyPoints, applyRanking, nextTeamTemplate } = queryHelpers;
  postFormUpdateCompetitionResults =
    postFormUpdateCompetitionResults as resultsGenerator.postFormUpdateCompetitionResults;
  ...
  const updateCompetition = async function () {
    ...
    if (
      ...
      await (updatedCompetition as any)
        .setTeams(relevantTeams, {
          transaction: t,
          through: { season: chosenSeason },
        })
  ...
  const latestCompetition = await updateCompetition().catch(function (
    err: Error
  ) {
    throw err;
  });
