...
const getCompetitionIndexData = async function (datahash = "") {
  try {
    ...
    const response = await fetch(`/competition/data/${datahash}`, {
      mode: "cors",
    ...
    if (response.ok) {
      ...
      const err = new Error(
        "Index data not available. Check your internet connection and try again."
      );
      throw err;
    }
  } catch (err) {
...
const toggleDataBySeason = async function (event) {
  ...
  if (options.includes(event.target)) {
    if (event.target.dataset.hash) {
      ...
      const data = await getCompetitionIndexData(hash).catch(function (err) {
        console.log(err);
        refreshArticle(err.message);
      });
